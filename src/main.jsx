...
function AuthScreen({ onAuthed }) {
  ...
  return <main className="auth-shell">
    ...
    <section className="auth-card">
      ...
      <p className="auth-subtitle">把两个人的时间，放在同一个清晰的画面里。</p>
    ...
    <div className="auth-footer">两个人的时间，更从容地并行。</div>
  </main>;
}
...
function TaskModal({ task, users, onClose, onSaved, onDeleted, toast }) {
  ...
  return <div className="modal-backdrop" onMouseDown={e => e.target === e.currentTarget && onClose()}><aside className="task-drawer">
    <header className="drawer-head"><div><span className="drawer-kicker">{isEdit ? '编辑日程' : '新建日程'}</span><h2>{isEdit ? (task.isPrivateMasked ? '私人安排' : task.title) : '安排一段好时间'}</h2></div><button className="icon-button" onClick={onClose}><X size={19} /></button></header>
    ...
    {tab === 'details' ? <form className="drawer-body" onSubmit={save}>
      <label className="label-block">标题<input autoFocus={!isEdit} placeholder="例如：一起买菜、专注写作" value={form.title} onChange={e => set('title', e.target.value)} /></label>
...
function MonthCalendar({ tasks, current, onSelect, onNew, onComplete }) {
  ...
  return <div className="month-view">
    ...
    <div className="month-grid">{days.map(day => {
      ...
      return <div key={key} className={`month-day ${inMonth ? '' : 'outside-month'} ${key === today ? 'is-today' : ''}`} onDoubleClick={() => onNew(key, '')}>
        <div className="month-day-number">{day.getDate()}</div>
        <div className="month-task-list">{dayTasks.slice(0, 5).map(task => <MonthTask key={task.occurrenceId} task={task} onSelect={onSelect} onComplete={onComplete} />)}{dayTasks.length > 5 && <div className="month-more">+{dayTasks.length - 5} 项</div>}</div>
      </div>;
    })}</div>
  </div>;
}
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><header className="topbar"><div className="breadcrumb"><span>{activePage === 'notifications' ? '通知' : '日历'}</span><ChevronRight size={14} /><strong>{activePage === 'notifications' ? '全部通知' : (viewMode === 'week' ? '本周安排' : '本月安排')}</strong></div><div className="top-actions">{activePage === 'notifications' ? <button className="primary-button add-button" onClick={() => setActivePage('calendar')}><CalendarDays size={16} />返回日历</button> : <><div className="search-box"><Search size={16} /><input placeholder="搜索日程、标签…" value={search} onChange={e => setSearch(e.target.value)} /></div><button className="icon-button notification-button" onClick={openNotifications}><Bell size={18} />{notifications.some(n => !n.read) && <i />}</button><button className="primary-button add-button" onClick={() => setModal({ date: dateKey(new Date()) })}><Plus size={17} />新建日程</button></>}</div></header>
      {activePage === 'notifications' ? <NotificationsPage notifications={notifications} onRead={markNotificationRead} onReadAll={markAllNotificationsRead} onBack={() => setActivePage('calendar')} /> : <section className="content-area"><div className="page-heading"><div><div className="eyebrow">{new Intl.DateTimeFormat('zh-CN', { year: 'numeric', month: 'long' }).format(current)}</div><h1>一起安排，刚刚好。</h1><p>看见彼此的时间，也留一点空间给自己。</p></div><div className="heading-actions"><div className="view-switch" aria-label="切换日历视图"><button className={viewMode === 'week' ? 'active' : ''} onClick={() => changeView('week')}>周</button><button className={viewMode === 'month' ? 'active' : ''} onClick={() => changeView('month')}>月</button></div><button className="today-button" onClick={goToday}>回到今天</button><div className="week-nav"><button className="icon-button" onClick={goPrevious}><ChevronLeft size={17} /></button><button className="icon-button" onClick={goNext}><ChevronRight size={17} /></button></div></div></div><div className="summary-strip"><div><span className="summary-icon teal"><CalendarDays size={17} /></span><div><small>{viewMode === 'week' ? '本周日程' : '本月日程'}</small><strong>{visibleTasks.length} <em>项</em></strong></div></div><div><span className="summary-icon orange"><Clock3 size={17} /></span><div><small>共同安排</small><strong>{visibleTasks.filter(t => t.assignment === 'both').length} <em>项</em></strong></div></div><div><span className="summary-icon purple"><CircleCheck size={17} /></span><div><small>已完成</small><strong>{visibleTasks.filter(t => t.status === 'done').length} <em>项</em></strong></div></div><div className="summary-tip"><span>✦</span><div><strong>给彼此留一点余地</strong><small>目前有 {visibleTasks.filter(t => t.priority === 'high' && t.status !== 'done').length} 项高优先级日程</small></div></div></div><div className="calendar-card">{viewMode === 'week' ? <Calendar tasks={visibleTasks} current={current} onMove={moveTask} onSelect={setModal} onNew={(date, start) => setModal({ date, start })} onComplete={complete} /> : <MonthCalendar tasks={visibleTasks} current={current} onSelect={setModal} onNew={(date, start) => setModal({ date, start })} onComplete={complete} />}</div></section>}
    </main>
