...
test('conflict API, notifications, privacy and recurrence lifecycle', async () => {
  ...
  try {
    ...
    assert.equal(backgroundTask.ignoreDayConflicts, true);
    assert.equal(backgroundTask.allDay, true);
    const withIgnored = await create(a, 'unrelated assigned event', { taskDate: '2026-09-23', startAt: at('2026-09-23', '10:00'), endAt: at('2026-09-23', '11:00') });
    assert.ok(!withIgnored.conflicts.some(c => c.tasks.some(task => task.id === background.task.id)));
    assert.ok(withIgnored.conflicts.some(c => c.tasks.some(task => task.id === first.task.id)), 'ignoring a background task does not suppress unrelated overlaps');
